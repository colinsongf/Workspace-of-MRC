...
class QAModel(object):
    ...
    def add_char_embeddings(self):
        ...
        with vs.variable_scope("char_embeddings"):
            ...
            self.context_char_embs = tf.reshape(self.context_char_embs, shape=(-1, self.FLAGS.word_max_len, self.FLAGS.char_embedding_size))  # shape = batch_size*context_len, word_max_len, char_embedding_size
            ...
            self.qn_emb_out = conv1d(input_=self.qn_char_embs, output_size=self.FLAGS.char_out_size, width=self.FLAGS.window_width, stride=1, scope_name='char-cnn') #reuse weights b/w context and ques conv layers
            ...
            self.qn_emb_out = tf.reshape(self.qn_emb_out, shape=(-1, self.FLAGS.question_len, self.FLAGS.char_out_size))  # Desired shape is Batch_size, question_len, char_out_size
    ...
    def build_graph(self):
        ...
        if self.FLAGS.do_char_embed:
            ...
            self.context_embs = tf.concat((self.context_embs, self.context_emb_out), axis=2)  # 字向量, 词向量 拼接
            print("Shape - concatenated context embs", self.context_embs.shape)

            self.qn_embs = tf.concat((self.qn_embs, self.qn_emb_out), axis=2)  # 字向量, 词向量 拼接
        ...
        """
        R-NET QUESTION CONTEXT ATTENTION and SELF ATTENTION 
        """
        if self.FLAGS.rnet_attention:
            """
            perform Question Passage and Self Matching attention from R-Net
            """
        ...
        """
        R-NET QUESTION POOLING and ANSWER POINTER
        """
