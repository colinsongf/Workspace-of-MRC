...
MAIN_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) # absolute path
print("main dir is {}".format(MAIN_DIR))
DEFAULT_DATA_DIR = os.path.join(MAIN_DIR, "1-data")  # relative path of data dir
EXPERIMENTS_DIR = os.path.join(MAIN_DIR, 'standford_squad/experiments') # relative path of experiments dir
...
tf.app.flags.DEFINE_bool("add_highway_layer", False, "Add highway layer to concatenated embeddings -True/False") # Default is True
...
tf.app.flags.DEFINE_integer("keep", 3, "How many checkpoints to keep. 0 indicates keep all (you shouldn't need to do keep all though - it's very storage intensive).")
...
tf.app.flags.DEFINE_string("data_dir", os.path.join(DEFAULT_DATA_DIR, 'SQuADv1.1'), "Where to find preprocessed SQuAD data for training. Defaults to data/")
...
def main(unused_argv):
    ...
    if FLAGS.mode == "train":
        ...
        with open(os.path.join(FLAGS.train_dir, "flags.json"), 'w') as fout:
            json.dump(FLAGS.__flags, fout)  # 保存训练参数信息
